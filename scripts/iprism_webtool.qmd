...
for measure in all_variables:  # Add more measures as needed
    ...
    # Count the number of rows per teamcode, then filter those that have more than 2
    teams = temp.groupby('teamcode').size().reset_index(name = "count").query("count > 1")

    # Count the number of observations where there are more than one entry per teamcode
    teams['count'].sum()

    # filter the icc data, i.e. create a subset
    ...
    # rename the Prism/Re-AIM measure to "measure" to assist in looping once
    # the data is sent to SAS
    df_to_sas["measure"] = df_to_sas[measure]

    # Drop the looping variable column, otherwise the length of the name is too
    # long for SAS.
    ...
    *ods select none;
    ...
    # Retrieve the SAS table icc_jun and load it int the Python env as a pandas
    # data frame
    sas_icc = sas.sasdata("icc_jun", libref = "work").to_df()
    
    # Calculate the Z value and the p value
    # n.b. the ZValue of the SAS table corresponds to the Estimate which is why
    # a Z value is calculated manually.
    sas_icc["z-value"] = sas_icc["icc"]/sas_icc["se_icc"]

    # Compute two-tailed p-value, returns a numpy array
    pval = 2 * (1 - norm.cdf(abs(sas_icc["z-value"])))

    # Format the p-value to 4 digits
    # Since pval is numpy array, index the 1st value as a float for formatting 
    pval = format_pval_flt(pval[0], 4)

    # Save the value in the table
    sas_icc["p-value"] = pval

    # Round columns
    cols_to_round = ['icc_LCL', 'icc_UCL', 'icc']
    sas_icc[cols_to_round] = sas_icc[cols_to_round].round(2)

    # Set the LCL to zero if it's negative, since an icc is not supposed to be
    # lower than 0, although mathematically it can be less than 0
    sas_icc["icc_LCL"] = sas_icc["icc_LCL"].clip(lower=0)
    
    # Select the columns of interest 
    sas_icc = sas_icc[["icc", "p-value", "icc_LCL", "icc_UCL"]]

    # Reset the index to the variable measure to display
