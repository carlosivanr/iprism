...
# Create a test df consisting of just the reach variable
...
test_df.to_csv("C:\\Users\\rodrica2\\OneDrive - The University of Colorado Denver\\Documents\\DFM\\projects\\iPRISM\\data\\icc_test.csv")
  ...
  # --- Step 1: Fit model function ---
def fit_model_and_get_vars(data):
  # Build the formula
  formula = f"{y} ~ 1"

  try:
      model = mixedlm(formula, data=data, groups=data[k])
      result = model.fit()
      bvar = result.cov_re.iloc[0, 0]        # Between-group (random intercept) variance
      wvar = result.scale                     # Residual (within-group) variance
      se_bvar = result.bse_re.iloc[0]         # Between-group standard error from model
      return bvar, wvar, se_bvar
  except Exception:
      return np.nan, np.nan, np.nan  # Handle convergence failures gracefully



# --- Step 2: Bootstrap procedure ---
def bootstrap_variance_components(test_df, n_boot=1000):
  
  # Initialize an empty list
  boot_results = []

  # Get the unique cluster value
  group_ids = test_df[k].unique()

  for _ in range(n_boot):


    # Sample clusters with replacement
    sampled_ids = np.random.choice(group_ids, size=len(group_ids), replace=True)

    # Concatenate a data frame consisting of values from the sample ids
    boot_df = pd.concat([test_df[test_df[k] == gid] for gid in sampled_ids])

    bvar, wvar, se_bvar = fit_model_and_get_vars(boot_df)

    # If the result is not a nan, then append results
    if not np.isnan(bvar):
        boot_results.append((bvar, wvar))

  return pd.DataFrame(boot_results, columns=["bvar", "wvar"])


# def get_iccs(data, y, k):
  """
  Parameters:
  y: string variable referencing the outcome column
  k: string variable referencing the clustering column
...
# --- Step 3: Calculate SEs and ICC ---
# Boot strap the model, to get n_boot bvars and wvars
# These will be used later on to calculated the std of the variance
# components. Once the variance components are estimated, they can 
# be used to calculate the variance of the ICC
boot_df = bootstrap_variance_components(test_df, n_boot=100)

# Drop failed fits (NaNs)
boot_df = boot_df.dropna()



# Point estimates from full data
bvar_hat, wvar_hat, se_wvar = fit_model_and_get_vars(test_df)


# Estimate SEs
se_bvar = boot_df["bvar"].std()
se_wvar = boot_df["wvar"].std()


# Compute ICC
icc = bvar_hat / (bvar_hat + wvar_hat)

# Delta method for SE of ICC
var_icc = se_bvar**2 * (wvar_hat**2 / (bvar_hat + wvar_hat)**4) + \
          se_wvar**2 * (bvar_hat**2 / (bvar_hat + wvar_hat)**4)

var_icc2 = se_bvar**2 * ((wvar_hat**2) / (bvar_hat + wvar_hat)**4) + \
    (se_wvar**2) * ((bvar_hat**2) / (wvar_hat + bvar_hat)**4)

se_icc = np.sqrt(var_icc)

# 95% CI
icc_lcl = icc - 1.96 * se_icc
icc_ucl = icc + 1.96 * se_icc

print(f"ICC: {icc:.3f}")
print(f"95% CI: [{icc_lcl:.3f}, {icc_ucl:.3f}]")
```
