...
```{python}
# Define a function to perform Steiger's Z-test
...
```{python}
#| eval: false
# FIRST DRAFT OF THE STEIGER Z TEST FOR LOOP BUT IT IS NOT RIGHT BC OF THE TRIPLICATES
# r_vars = prefixed_df.columns[0:4].tolist()

# # Empty list to store results
# steiger_results = []

# for r_var in r_vars:
#     # Name of vars without the r_var
#     remaining_vars = r_vars.copy()
#     remaining_vars.remove(r_var)

#     # Count the number of elements in remaining_vars

#     # For loop, for every element in remaining vars
#     # get the correlation between r_var and remaining_var[i]
#     subset_a = prefixed_df[[r_var, remaining_vars[0]]].dropna()
#     r_a, p_a = spearmanr(subset_a.iloc[:,0], subset_a.iloc[:,1])

#     subset_b = prefixed_df[[r_var,  remaining_vars[0], remaining_vars[1]]].dropna()
#     r_b, p_b = spearmanr(subset_b.iloc[:,0], subset_b.iloc[:,1])

#     #
#     subset_c = prefixed_df[[remaining_vars[0], remaining_vars[1]]].dropna()
#     r_c, p_c = spearmanr(subset_c.iloc[:,0], subset_c.iloc[:,1])

#     # *** Still need to figure out how to get the subsets right,
#     # they need to be in the lowest number of complete cases in 
#     # triplicates
#     # Steigers would go here.




#     for r_var2 in r_vars:
#         # Compute Spearman correlation
#         subset = prefixed_df[[r_var, r_var2]].dropna()

#         # Get the ids to subset all remaining columns using
#         # the same exact rows
#         ids = subset.index

#         # Calculate all of the pairwise correlations
#         corr_mat = prefixed_df.loc[ids].corr(method = "spearman")




#         # Get the first corr value
#         r_value, p_value = spearmanr(subset.iloc[:,0], subset.iloc[:,1])
...
```{python}
# Create a test loop to display the combinations
r_vars = prefixed_df.columns[0:4].tolist()

for i in range(len(r_vars)):
    remaining_vars = r_vars.copy()
    remaining_vars.remove(r_vars[i])

    for j in range(len(remaining_vars)):
      remaining_vars_2 = remaining_vars.copy()
      remaining_vars_2.remove(remaining_vars[j])
      
      for k in range(len(remaining_vars_2)):
        print([r_vars[i], remaining_vars[j], remaining_vars_2[k]])


        # Get the triplicate subset to maximize the n
        subset = prefixed_df[[r_vars[i], remaining_vars[j], remaining_vars_2[k]]].dropna()

        # Specify the triplicate
        triplicate = [r_vars[i], remaining_vars[j], remaining_vars_2[k]]

        # Sort the triplicate to make a label
        triplicate.sort()

        # Label
        label = '-'.join(triplicate)

        # *** Will need the actual triplicate for the order of the correlations
        # AND the label to avoid having to conduct un-necessary correlations or
        # filter out

        # correlations, the [0] indexes just the r value and not the p value
        r1 = spearmanr(subset.iloc[:,0], subset.iloc[:,1])[0]
        r2 = spearmanr(subset.iloc[:,0], subset.iloc[:,2])[0]
        r3 = spearmanr(subset.iloc[:,1], subset.iloc[:,2])[0]


        # Steiger's Z-test goes here, after having r1, r2, and r3

        # Could add a way to sort the triplicates, and if it is not in the results data frame
        # conduct the z-test and append to results data frame, else skip to the next
